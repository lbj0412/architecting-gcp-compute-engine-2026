...
<div class="cover-guide-box">
  Compute Engine VM 인스턴스, 머신 시리즈(C3/C4/N4), 스토리지(Persistent Disk/Hyperdisk/Local SSD), Live Migration 및 비용 최적화 종합 가이드
</div>
...
## Compute Engine 스토리지 옵션: Persistent Disk & SSD
...
## Spot VM을 활용한 극강의 비용 절감 (Spot VM FinOps)
...
<div class="card" style="margin-top: 20px; padding: 30px;">
  <h3 style="color: #202124; font-size: 24px; margin-top:0;">❓ Q1. Google Cloud의 Live Migration 기능에 대한 올바른 설명은 무엇인가요?</h3>
  <p style="font-size: 20px; color: #3c4043; line-height: 1.8;">
    A. 물리 서버 점검 시 VM이 자동으로 강제 재부팅된다.<br>
    B. <strong>물리 서버 유지보수 중에도 VM을 서비스 중단이나 재부팅 없이 다른 서버로 실시간 무중단 이동시킨다.</strong><br>
    C. VM의 모든 데이터를 구글 드라이브로 자동 업로드한다.<br>
    D. Spot VM에서만 작동하는 전용 기능이다.
  </p>
</div>
...
comment:
💬 [강사 대본]
"모듈 03 첫 번째 복습 퀴즈입니다. Live Migration 기술의 가장 핵심적인 특징을 골라보세요."
-->
...
comment:
💬 [강사 대본]
"정답은 B번입니다! Live Migration은 무중단 실시간 이동 기술이라는 점을 명심해 주세요."
-->
